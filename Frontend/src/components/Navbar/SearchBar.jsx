...
export default function SearchBar({ onSelectStop }) {
  ...
  const handleKeyDown = (e) => {
    ...
    if (e.key === 'ArrowDown') {
      ...
      e.preventDefault();
      setHighlightIndex((prev) => Math.max(prev - 1, 0));
    } else if (e.key === 'Enter' && highlightIndex >= 0 && filteredStops[highlightIndex]) {
      e.preventDefault();
      handleSelectStop(filteredStops[highlightIndex]);
    }
  };
  ...
  return (
    <Box sx={{
      ...
      {loading && (
        <CircularProgress size={20} sx={{ position: 'absolute', right: 20, top: 18 }} />
      )}
      {error && (
        <Typography variant="caption" color="error" sx={{ mt: 1, px: 2 }}>{error}</Typography>
      )}
      ...
      {showDropdown && filteredStops.length > 0 && (
        <Popper open={showDropdown} anchorEl={anchorEl} placement="bottom-start" sx={{ width: '100%', zIndex: 1200 }}>
          <Paper sx={{
            mt: 1,
            borderRadius: 2,
            boxShadow: 3,
