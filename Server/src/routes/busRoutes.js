...
const { getBuses, createBus, updateBus, deleteBus, logTrip, getPublicBuses, updateBusLocation, getBusById } = require('../controllers/busController');
...
// 🔒 Protected driver route for location updates
router.post('/:id/location', authMiddleware, checkRole(['driver']), updateBusLocation);

// 🔒 New protected route to fetch a single bus by ID
router.get('/single/:id', authMiddleware, checkRole(['admin', 'super-admin', 'driver']), getBusById);
